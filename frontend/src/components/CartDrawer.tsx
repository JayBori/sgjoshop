...
export default function CartDrawer({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    ...
    >
      ...
      >
        <strong>장바구니</strong>
        <button className="btn" onClick={onClose} style={{ background: "#eee", color: "#111" }}>
          닫기
        </button>
      </div>
      <div style={{ padding: 16, overflow: "auto", height: "calc(100% - 180px)" }}>
        {state.items.length === 0 && <div style={{ color: "#666" }}>장바구니가 비어있습니다.</div>}
        {state.items.map((it) => (
          <div key={it.product_id} style={{ display: "flex", gap: 8, marginBottom: 12 }}>
            ...
            <button
              className="btn"
              onClick={async () => {
                await api.remove(it.product_id);
                await refresh();
              }}
      ...
      <div style={{ padding: 16, borderTop: "1px solid #e5e7eb" }}>
        ...
        {state.discount ? (
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
            <span>할인{state.coupon ? ` (${state.coupon})` : ""}</span>
        ...
        <a className="btn" style={{ width: "100%", display: "inline-block", textAlign: "center" }} href="/cart">
          장바구니 보기/쿠폰 적용
        </a>
        <button
          className="btn"
          ...
          onClick={async () => {
            try {
              ...
              alert(`주문이 생성되었습니다. 주문번호: ${r.order_id}`);
              await refresh();
            } catch (e: any) {
