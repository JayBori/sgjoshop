...
export default function AdminPage(){
  ...
  async function loadOrders(t:string){
    const r = await fetch(`${getApiBase()}/admin/orders`, { headers: { Authorization: `Bearer ${t}` } });
  ...
  async function toggleUser(uid:number, nextActive:boolean){
    ...
    fd.append('active', nextActive? '1':'0')
  ...
  // Settings save (simple key/value JSON payload)
  ...
  return (
    <main className="container">
      ...
      <div style={{marginBottom:12, display:'flex', gap:8, flexWrap:'wrap'}}>
        ...
        <button onClick={()=>setTab('media')}>미디어</button>
      ...
      {tab==='categories' && (
        <section>
          ...
          <table><thead><tr><th>ID</th><th>이름</th><th>Slug</th><th>정렬</th><th>동작</th></tr></thead><tbody>
            {categories.map(c=> (
              <tr key={c.id}>
                <td>{c.id}</td>
                <td colSpan={3}>
      ...
      {tab==='coupons' && (
        <section>
          ...
          <table><thead><tr><th>ID</th><th>코드</th><th>타입</th><th>값</th><th>활성</th><th>기간</th><th>최소</th><th>동작</th></tr></thead><tbody>
            {coupons.map(c=> (
              <tr key={c.id}>
                <td>{c.id}</td>
                <td colSpan={6}>
