      ...
      {tab==='users' && (
        <section>
          ...
          <table>
            ...
            <tbody>
              {users.map(u=> (
                <tr key={u.id}><td>{u.id}</td><td>{u.username}</td><td>{u.is_active? 'Active':'Inactive'}</td><td>{u.is_admin? 'Admin':'User'}</td><td>{u.must_change_password? 'Y':'N'}</td><td>{u.last_login || '-'}</td><td><button onClick={()=>toggleUser(u.id, !u.is_active)}>{u.is_active? '비활성':'활성'}</button><button onClick={()=>deleteUser(u.id)} style={{marginLeft:8}}>삭제</button></td></tr>
              ))}
            </tbody>
          </table>
        </section>
